...
def test_cf_1_5_5_relax():
    ...
    assert np.all(
        [
            my_data.df.iloc[i]["cLIS_lig_rec"]
            == pytest.approx(expected_cLIS_pep_rec[i], precision)
            for i in range(len(my_data.df))
        ]
    )
    ...
    assert np.all(
        [
            my_data.df.iloc[i]["cLIS_rec_lig"]
            == pytest.approx(expected_cLIS_rec_pep[i], precision)
            for i in range(len(my_data.df))
        ]
    )
